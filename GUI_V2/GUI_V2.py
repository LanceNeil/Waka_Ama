...
import csv_export as csv_export
...
class program_functions_component():
    ...
    def check_parent_folder_is_valid(self):
        ...
        if parent_folder_items:
            ...
            return f"No WakaNats folder/file inside the {self.parent_folder}"

        else:
            return f"There are no items inside the {self.parent_folder}"
    ...
    def check_target_year_is_valid(self):
        ...
        if self.target_year.isdigit():
            ...
            if self.target_year in all_available_years:
                ...
                return f"The year {self.target_year} is not found"


        else:
            return f"{self.target_year} is not a year (Use a number)"
...
class gui_component():
    ...
    def home_screen(self):
        ...
        # help button
        # -- width 0 to maximize
        help_button = ct.CTkButton(homepage_frame,text="?",width=0,command=self.help_screen) # maximize the button width to the text
        help_button.grid(row=0,column=0,padx=10,pady=10,sticky="ne") # displaces the help button on the 'north east'

        

        # title 
        title_label = ct.CTkLabel(homepage_frame,text="Waka Ama Extractor",font=self.title_font) # Creates the title with its text
        ...
        # Open folder
        self.parent_folder_button = ct.CTkButton(inner_frame,text="Select Folder",command=self.pick_folder) # Creates a button to open folder
        self.parent_folder_button.grid(row=0,column=0) # displays the open folder
        

        # Keyword input
        self.keyword_input = ct.CTkEntry(inner_frame,placeholder_text="Keyword") # Creates an input box for the keyword
        self.keyword_input.grid(row=0,column=1) # displays the keyword input box
        self.processed_keyword = self.keyword_input.get()

        # Year input
        self.year_input = ct.CTkEntry(inner_frame,placeholder_text="Year") # Creates the year input box
        self.year_input.grid(row=1,column=1) # displays the year input box
        ...
        self.save_to_csv_switch.grid(row=1,column=0) # displays save to csv switch

        # proceed
        proceed_button = ct.CTkButton(inner_frame,text="Proceed",command=self.loading_screen) # Creates the proceed button
        proceed_button.grid(row=2,column=0,columnspan=2) #displays the proceed button
    ...
    def loading_screen(self):
        ...
        # create the title
        loading_title_label = ct.CTkLabel(loading_frame, text=f"Analysing File... {self.year_input.get()} {self.keyword_input.get()}") # creates the loading title
    ...
    def help_screen(self):
        ...
        help_statement = ct.CTkLabel(help_frame,text="Select Folder: Select the Parent Folder that contains the files that you want to analyse.\n"
                                                       "Keyword: Type the words Semi or Final so that the program only reads the files with the name on the .lif.\n"
                                                       "Year: Type the year of the folder that you want to analyse (2017 or 2018).")
        help_statement.grid()
    ...
    def pick_folder(self):
        ...
        if folder_path: # checks if folder exists
            ...
            self.file_error_dialog(error_type="folder",folder=folder_path) # open error dialog. Folder error and the messaeg "Cannot open folder"
